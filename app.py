...
openai.api_key = "OPENAI API KEY"#replace this with ur open api key.
app = Flask(__name__)
...
def chatbot():
    ...
    handbook_path = 'docs/DMV_handbook.pdf'
    handbook_text = extract_pdf_text(handbook_path)
    summary_ids = t5_tokenizer.encode(handbook_text, max_length=1024, truncation=True, return_tensors='pt')
    summary_ids = summary_ids.to(t5_device)
    ...
    bot_response = generate_gpt3_answer("\n".join(prompts) + "\nUser: " + user_input_text + "\nBot:")
    return jsonify({"user_input": user_input_text, "response": bot_response})
